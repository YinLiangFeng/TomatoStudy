...
16. 实现点击“开始”进入 `FocusActivity`。（已完成）
17. 实现专注计时、暂停、继续、结束。（已完成）
18. 实现结束专注后保存 `focus_record`。（已完成）
19. 实现休息倒计时。（已完成）
20. 实现休息结束或主动结束后返回首页。（已完成）
21. 实现统计数据页 5 个模块。
    - 21.1 实现统计数据页的“累计专注”模块。（已完成）
    - 21.2 实现统计数据页的“当日专注”模块。（已完成）
    - 21.3 实现统计数据页的“专注时长分布”模块，使用饼状图展示。（已完成）
    - 21.4 实现统计数据页的“本月专注时段分布”模块，使用柱状图展示。（已完成）
    - 21.5 实现统计数据页的“月度专注统计”模块，使用折线图展示。（已完成）
22. 实现学习打卡模块。
23. 实现我的页面和设置页面。
24. 实现本地通知提醒。
